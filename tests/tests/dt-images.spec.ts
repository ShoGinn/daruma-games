...
describe('checkImageExists', () => {
	...
	it('returns false for an invalid URL', async () => {
		...
		const result = await checkImageExists(
...
describe('getAssetUrl', () => {
	...
	it('returns the ipfs hosted url', async () => {
		...
		asset.url =
			'ipfs://bafybeihmsmcpvdphzqcvghq4anic64avbrimxskkulogj6wtijmfnk3b24#i';
	...
	it('returns the hosted url for a video asset with #v', async () => {
		...
		const asset = new AlgoNFTAsset(1, creatorWallet, 'test', 'test', 'test');
		asset.url =
			'ipfs://bafybeihevpbpqfvzwqpyahx3gid7gkaeex7u6fqh4z7jjwhtbcpz3cltoe#v';
		asset.arc69 = {
			standard: 'arc69',
			mime_type: 'video/mp4',
			properties: {
	...
	it('returns the hosted url for a video asset with video/mp4 mime type', async () => {
		...
		const asset = new AlgoNFTAsset(1, creatorWallet, 'test', 'test', 'test');
		asset.url =
			'ipfs://bafybeihevpbpqfvzwqpyahx3gid7gkaeex7u6fqh4z7jjwhtbcpz3cltoe';
		asset.arc69 = {
			standard: 'arc69',
			mime_type: 'video/mp4',
			properties: {
	...
	it('returns the url if its just a url', async () => {
		...
		const url = await getAssetUrl(asset);
		expect(url).toBe(
			'https://shoginn.github.io/daruma-images/game/npc/OneVsNpc.gif',
		);
	});
