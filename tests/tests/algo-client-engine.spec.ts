...
describe('AlgoClientEngine', () => {
	...
	it('logs the correct default connection types', () => {
		...
		expect(algodClient.c.bc.baseURL.toString()).toMatch(
	...
	it('successfully runs a RateLimitedRequest', async () => {
		...
		await expect(api.testRateLimitedRequest(mockRequest)).resolves.toBe(
			'response',
		);
	});
	...
	it('limits the rate of requests', async () => {
		...
		await expect(
			_algoClientEngine.testRateLimitedRequest(mockRequest),
		).rejects.toThrow(
