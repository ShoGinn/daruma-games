...
describe('webhook', () => {
	...
	it('should create a transaction webhook message', () => {
		...
		expect(
			(mockSentEmbeds[0] as { data: { title: string } }).data.title,
		).toEqual('Claimed (KARMA) -- Algorand Network Transaction');
	});
	it('should create a karma artifact webhook message', () => {
		...
		expect(
			(mockSentEmbeds[0] as { data: { title: string } }).data.title,
		).toEqual('Artifact Claimed -- Algorand Network Transaction');
	});
