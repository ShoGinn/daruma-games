...
describe('User tests that require db', () => {
	...
	describe('removeWalletFromUser', () => {
		...
		it('should not remove the wallet', async () => {
			...
			const result = await userRepo.removeWalletFromUser(
				user.id,
				wallet2.address,
			);
		...
		it('should remove the wallet even if the user has multiple wallets', async () => {
			...
			const { wallet: userWallet2 } = await addRandomAssetAndWalletToUser(
				database,
				user,
			);
			const { wallet: userWallet3 } = await addRandomAssetAndWalletToUser(
				database,
				user,
			);
		...
		it('should not remove the wallet if the user has unclaimed tokens', async () => {
			...
			const getTokenFromAlgoNetwork = jest.spyOn(
				algoStdTokenRepo,
				'getTokenFromAlgoNetwork',
			);
			...
			const removed = await userRepo.removeWalletFromUser(
				user.id,
				wallet.address,
			);
			expect(removed).toBe(
