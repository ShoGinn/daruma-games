...
describe('asset tests that require db', () => {
  ...
  describe('Clearing the cool down for assets', () => {
    test('should clear a cooldown for one user', async () => {
      ...
      const newAsset = await database.getRepository(AlgoNFTAsset).findOneOrFail({ id: asset.id });
  ...
  describe('addWalletAssets', () => {
    ...
    test('should not add assets to a wallet and should not remove any because asset-id does not match any asset in wallet', async () => {
      ...
      const assets = await algoWallet.addWalletAssets(wallet.address, [blankAsset, ownedAsset]);
      expect(assets?.assetsAdded).toBe(0);
      expect(assets?.assetsRemoved).toBe(0);
