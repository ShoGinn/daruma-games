...
describe('asset tests that require db', () => {
  ...
  test('Add a channel then then remove it -- Expect None', async () => {
    expect.assertions(4);
    const guild = await addRandomGuild(database, channel.guildId);
  ...
  test('Add a channel then try and add it again -- Expect channel to be returned', async () => {
    expect.assertions(1);
  ...
  describe('getChannelMessageId function', () => {
    test('expect undefined when no channel', async () => {
    ...
    test('expect message id when channel', async () => {
      await addRandomGuild(database, channel.guildId);
  ...
  test('Add a channel then update the message id', async () => {
    await addRandomGuild(database, channel.guildId);
    const addedChannel = await dtChannelRepo.addChannel(channel, GameTypes.OneVsOne);
  ...
  test('check all the ways getGuild works', async () => {
    expect.assertions(3);
    const guild = await addRandomGuild(database, channel.guildId);
    await dtChannelRepo.addChannel(channel, GameTypes.OneVsOne);
