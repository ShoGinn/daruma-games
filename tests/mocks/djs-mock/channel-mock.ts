...
import { sortMessagesById } from '../../../src/utils/functions/message.js';
import {
	isSnowflakeLarger,
...
>(type: Type, guild: Guild): APIGuildTextChannel<Type> {
...
export function mockNewsChannel(input: {
	...
	return setupMockedChannel(input.client, input.guild, (guild) => {
		...
		return Reflect.construct(NewsChannel, [
			guild,
			rawData,
			input.client,
		]) as NewsChannel;
...
export function mockMessageReaction({
	...
	return Reflect.construct(MessageReaction, [
		message.client,
		data,
		message,
	]) as MessageReaction;
