...
export default class WalletCommand {
	...
	async userSync(
		...
		const message = await em
			.getRepository(User)
			.syncUserWallets(interaction.targetId);
	...
	async userCoolDownClear(
		...
		await em
			...
			.clearAssetCoolDownsForUser(interaction.targetId);
		await InteractionUtils.replyOrFollowUp(interaction, {
	...
	async removeWallet(interaction: ButtonInteraction): Promise<void> {
		...
		const message = await em
			.getRepository(User)
			.removeWalletFromUser(discordUser, address);
	...
	private async sendWalletEmbeds({
		...
		const wallets =
			(await em
				.getRepository(AlgoWallet)
				.getAllWalletsByDiscordId(discordUser)) ?? [];
		...
		for (let index = 0; index < wallets.length; index++) {
			...
			const customizeDaruma = customButton(
				discordUser,
				'Customize your Daruma',
			);
	...
	private async getWalletEmbed({
		...
		const embed = new EmbedBuilder()
			.setThumbnail(
				await em
					.getRepository(AlgoWallet)
					.getRandomImageUrl(currentWallet.address),
			)
			.setAuthor({
				name: user.username,
		...
		const nfDomain = await nfDomainsMgr.getWalletDomainNamesFromWallet(
			currentWallet.address,
		);
		let nfDomainString = '';
	...
	async userSyncWallet(interaction: ButtonInteraction): Promise<void> {
		...
		const message = await em
			.getRepository(User)
			.syncUserWallets(interaction.user.id);
		await InteractionUtils.replyOrFollowUp(interaction, message);
	}
