...
export default class KarmaCommand {
	...
	async add(
		...
		if (walletWithMostTokens) {
			...
			await InteractionUtils.replyOrFollowUp(
				interaction,
				`User ${karmaAddUser.toString()} does not have a wallet to add ${this
					.gameAssets.karmaAsset?.name} to`,
			);
			return;
		}
	...
	@Slash({
		description:
			'Send Karma Immediately to a user (they do not have to claim!)',
		name: 'send',
	})
	@Category('Admin')
	@SlashGroup('karma')
	async send(
		...
		@SlashOption({
			description:
				'How Much are you Sending? (Bot uses the wallet with the most KARMA)',
			name: 'amount',
			required: true,
			type: ApplicationCommandOptionType.Number,
		})
	...
	async tip(
		...
		@SlashOption({
			description:
				'How Much are you Tipping? (Bot uses the wallet with the most KARMA)',
			name: 'amount',
			required: true,
			type: ApplicationCommandOptionType.Number,
		})
	...
	async claim(interaction: CommandInteraction): Promise<void> {
		...
		for (const wallet of optedInWallets) {
			...
			if (unclaimedKarma && unclaimedKarma?.unclaimedTokens > 0) {
				...
				walletsWithUnclaimedKarmaTuple.push([
					wallet,
					unclaimedKarma.unclaimedTokens,
				]);
			}
		}
		...
		if (walletsWithUnclaimedKarma.length > 0) {
			...
			for (const wallet of walletsWithUnclaimedKarmaTuple) {
				walletsWithUnclaimedKarmaFields.push({
					...
					value: `${wallet[1].toLocaleString()} ${this.gameAssets.karmaAsset
						?.name}`,
					inline: true,
				});
			}
		...
		const walletDesc =
			walletsWithUnclaimedKarma.length > 1 ? greaterThanOneWallet : oneWallet;
		...
		collector.on('collect', async (collectInteraction: ButtonInteraction) => {
			...
			if (collectInteraction.customId.includes('yes')) {
				...
				for (const wallet of walletsWithUnclaimedKarmaTuple) {
					...
					if (claimStatus.txId) {
						logger.info(
							`Claimed ${claimStatus.status?.txn.txn.aamt ?? ''} ${this
								.gameAssets.karmaAsset?.name} for ${caller.user.username} (${
								caller.id
							})`,
						);
						claimEmbedFields.push(
							...
							},
							{
								name: 'Txn Hash',
								value:
									claimStatus.status?.['confirmed-round']?.toString() ?? 'N/A',
							},
							{
								name: 'Transaction Amount',
								value:
									claimStatus.status?.txn.txn.aamt?.toLocaleString() ?? 'N/A',
							},
						);
	...
	async shop(interaction: CommandInteraction): Promise<void> {
		...
		// Get the shop embed
		const karmaOptedIn = await this.getOptedInWallets(
			interaction,
			this.gameAssets.karmaAsset,
		);
		if (!karmaOptedIn) {
		...
		collector.on('collect', async (collectInteraction: ButtonInteraction) => {
			...
			switch (collectInteraction.customId) {
				...
				case 'buyArtifact': {
					...
					if (claimStatus.txId) {
						shopEmbed.setImage(
							optimizedImageHostedUrl(OptimizedImages.ARTIFACT),
						);
						shopEmbed.addFields(
				...
				case 'buyEnlightenment': {
					...
					if (claimStatus.txId) {
						shopEmbed.setImage(
							optimizedImageHostedUrl(OptimizedImages.ENLIGHTENMENT),
						);
						shopEmbed.addFields(
	...
	async claimArtifact(
		...
		if (claimStatus.txId) {
			logger.info(
				...
				} ${this.gameAssets.karmaAsset?.name} for ${caller.user.username} (${
					caller.id
				})`,
			);
	...
	async claimEnlightenment(
		...
		if (claimStatus.txId) {
			...
			await userDatabase.updateUserPreToken(
				caller.id,
				-this.necessaryArtifacts,
			);
			await userDatabase.syncUserWallets(caller.id);
	...
	async shopEmbed(discordUserId: string): Promise<{
		...
		const {
			walletWithMostTokens: userClaimedKarmaWallet,
			unclaimedTokens: unclaimedKarma,
		} = await em
			.getRepository(AlgoWallet)
		...
		const userClaimedKarmaStdAsset =
			await algoStdTokenDatabase.getStdAssetByWallet(
				userClaimedKarmaWallet,
				this.gameAssets.karmaAsset?.id,
			);
		const userEnlightenmentStdAsset =
			await algoStdTokenDatabase.getStdAssetByWallet(
				userClaimedKarmaWallet,
				this.gameAssets.enlightenmentAsset.id,
			);
		...
		if (
			userClaimedKarma >= this.artifactCost ||
			totalPieces >= this.necessaryArtifacts
		) {
		...
		shopEmbed.setTitle(
			`Welcome to The ${this.gameAssets.karmaAsset?.name} Shop`,
		);
		...
		const shopEmbedFields = this.noArmsOrLegs
			? [justArtifactsField]
			: armsAndLegsFields;
		...
		if (
			totalPieces >= this.necessaryArtifacts &&
			userEnlightenmentStdAsset?.optedIn
		) {
		...
		collector.on('collect', async (collectInteraction: ButtonInteraction) => {
			...
			switch (collectInteraction.customId) {
				case 'uptoFiveCoolDown': {
					...
					shadyEmbeds.setDescription(
						'Buying an elixir for 5 cool downs... maybe...',
					);
			...
			if (thisStatus.claimStatus.txId) {
				shadyEmbeds.addFields(
					ObjectUtil.singleFieldBuilder('Elixir', 'Purchased!'),
				);
				shadyEmbeds.addFields(
	...
	> {
		...
		// Build the embed
		const tenorUrl = await this.tenorManager.fetchRandomTenorGif(
			'trust me, shady, scary',
		);
	...
	async claimElixir(
		...
		const { walletWithMostTokens: rxWallet } =
			await algoWalletDatabase.allWalletsOptedIn(
				caller.id,
				this.gameAssets.karmaAsset,
			);
		if (!rxWallet) {
		...
		if (claimStatus.txId) {
			logger.info(
				`Elixir Purchased ${claimStatus.status?.txn.txn.aamt ?? ''} ${this
					.gameAssets.karmaAsset?.name} for ${caller.user.username} (${
					caller.id
				})`,
			);
			resetAssets = await algoWalletDatabase.randomAssetCoolDownReset(
				caller.id,
				coolDowns,
			);
			await userDatabase.syncUserWallets(caller.id);
