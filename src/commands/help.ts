	...
	private displayCategory(
		...
		if (category === 'categories') {
			const embed = new EmbedBuilder()
				...
				.setColor('#0099ff')
				.setDescription(
					`The items shown below are all the commands supported by this bot`,
				)
				.setFooter({
		...
		const embed = new EmbedBuilder()
			...
			.setColor('#0099ff')
			.setFooter({
				text: `${client.user?.username ?? 'Bot'} • Page ${
					pageNumber + 1
				} of ${maxPage}`,
			})
			.setTimestamp();
		...
		for (const item of resultOfPage) {
			...
			const fieldValue = isString(itemDescription)
				? itemDescription
				: 'No description';

			const resultName = isString(itemGroup)
				? `/${itemGroup} ${itemName}`
				: `/${itemName}`;
