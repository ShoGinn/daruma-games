...
export const ContextMenu = (options: ContextMenuOptions) => {
  ...
  if (options.localizationSource)
    ...
    localizationSource = ('COMMANDS.' +
      constantPreserveDots(options.name)) as TranslationsNestedPaths
  else if (commandNameFromFile)
    localizationSource = ('COMMANDS.' +
      constantPreserveDots(commandNameFromFile)) as TranslationsNestedPaths
