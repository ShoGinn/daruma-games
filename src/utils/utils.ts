...
export class ObjectUtil {
	...
	public static async randomDelayFor(
		minDelay: number,
		maxDelay: number,
	): Promise<void> {
		const delay =
			minDelay === maxDelay
				? minDelay
				: randomInt(Math.min(minDelay, maxDelay), maxDelay);
	...
	public static convertBigIntToNumber(
...
export async function sendMessageToAdminChannel(
	message: string,
	client: Client,
): Promise<boolean> {
