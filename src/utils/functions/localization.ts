...
export const setOptionsLocalization = <
...
export const getLocalizationFromPathString = (
  path: TranslationsNestedPaths,
  locale?: Locales
) => {
  const pathArray = path?.split('.') || []
  let currentLocalization: any =
    loadedLocales[locale ?? generalConfig.defaultLocale]

  for (const pathNode of pathArray) {
    currentLocalization =
      currentLocalization[pathNode as keyof typeof currentLocalization]
