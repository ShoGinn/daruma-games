	...
	if (missingEmojis.length > 0) {
		logger.warn(
			`Missing emojis: ${missingEmojis.join(
				', ',
			)}. Using default emojis instead.`,
		);
	}
}

export function getGameEmoji(
	damageOrOther?: number | string | undefined,
): string {
	if (
	...
	return (
		emojis.get(`${damageOrOther}png`) ?? emojiConvert(damageOrOther.toString())
	);
}
