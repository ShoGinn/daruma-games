...
async function getUserMention(userId: string): Promise<string> {
	try {
		...
		logger.error(
			`Error getting user mention for ID ${userId}: ${JSON.stringify(error)}`,
		);
		return `<@${userId}>`;
	}
}
...
export async function doEmbed<T extends EmbedOptions>(
	...
	const playerArrayFields = async (
		...
		const fieldPromises = playerArray_.map(async (player, index) => {
			const userMention = player.isNpc
				? 'NPC'
				: await getUserMention(player.dbUser.id);
			...
			const winnerCheckBox = gameFinished
				? player.isWinner
					...
					: '❌'
				: '';
			const playerNumber = `${winnerCheckBox} ${emojiConvert(
				(index + 1).toString(),
			)}`;
			const embedMessage = [
				playerNumber,
	...
	switch (gameStatus) {
		...
		case GameStatus.win: {
			...
			embed
				.setDescription(
					`${assetName(player.playableNFT)} ${sampledWinningReasons}`,
				)
		...
		case GameStatus.maintenance: {
			const tenorUrl =
				await tenorImageManager.fetchRandomTenorGif('maintenance');
			embed
...
function walletSetupButton(): Array<
	ActionRowBuilder<MessageActionRowComponentBuilder>
> {
...
export async function allDarumaStats(
	...
	const rankedAssets = await database
...
export async function coolDownModified(
	player: Player,
	orgCoolDown: number,
...
function randomCoolDownOfferButton(): Array<
	ActionRowBuilder<MessageActionRowComponentBuilder>
> {
...
function showCoolDownsButton(): Array<
	ActionRowBuilder<MessageActionRowComponentBuilder>
> {
...
async function getRemainingPlayableDarumaCountAndNextCoolDown(
	...
	const assetsInCoolDown = filterNotCooledDownOrRegistered(
		allAssets,
		interaction.user.id,
		games,
	);
	...
	assetsInCoolDown.sort(
		(a, b) => a.dojoCoolDown.getTime() - b.dojoCoolDown.getTime(),
	);
...
export async function flexDaruma(
	interaction: ButtonInteraction,
): Promise<void> {
	const database = container.resolve(MikroORM).em.fork();
...
export async function registerPlayer(
	...
	const caller = await InteractionUtils.getInteractionCaller(interaction);
	const assetId = randomDaruma
		? randomDaruma.id.toString()
		: customId.split('_')[1] || '';
	...
	const ownerWallet = await userAssetDatabase.getOwnerWalletFromAssetIndex(
		userAsset.id,
	);
	...
	const optedIn = await stdTokenDatabase.isWalletWithAssetOptedIn(
		ownerWallet,
		karmaAsset.id,
	);
	if (!optedIn) {
	...
	const remainingPlayableDarumaLengthMessage = `${
		remainingPlayableDaruma > 0
			? inlineCode(remainingPlayableDaruma.toLocaleString())
			: 'No'
