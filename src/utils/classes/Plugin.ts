...
export class Plugin {
  ...
  public async load(): Promise<void> {
    ...
    if (
      !semver.satisfies(
        generalConfig.__templateVersion,
        pluginConfig.tscordRequiredVersion
      )
    )
      return this.stopLoad(
        `Incompatible with the current version of Tscord (v${generalConfig.__templateVersion})`
      )
  ...
  private stopLoad(error: string): void {
    this._valid = false
    console.error(
      `Plugin ${this._name ? this._name : this._path} ${
        this._version ? 'v' + this._version : ''
      } is not valid: ${error}`
    )
  }
  ...
  private async getEntities(): Promise<{
    [key: string]: EntityClass<AnyEntity>
  }> {
  ...
  private async getTranslations(): Promise<{ [key: string]: BaseTranslation }> {
    ...
    for (const defaultLocale of locales) {
      const path = `${getSourceCodeLocation()}/i18n/${defaultLocale}/${
        this._name
      }/_custom.`
