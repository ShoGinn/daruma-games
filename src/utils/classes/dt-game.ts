...
export class Game {
	...
	async addNpc(): Promise<boolean> {
		...
		const [botCreator, asset] = await Promise.all([
			em
				.getRepository(User)
				.findOne({ id: InternalUserIDs.botCreator.toString() }),
	...
	addPlayer(player: Player): boolean {
		if (this.getPlayer(player.dbUser.id)) {
			if (
				this.getPlayer(player.dbUser.id)?.playableNFT.id !=
				player.playableNFT.id
			) {
	...
	async saveEncounter(): Promise<void> {
		...
		const em = this.orm.em.fork();
		const encounter = await em
			.getRepository(DtEncounters)
			.createEncounter(this);
	...
	checkForWin(): void {
		...
		if (
			currentPlayer &&
			roundIndex === gameWinRoundIndex &&
			rollIndex === gameWinRollIndex
		) {
	...
	async checkWaitingRoomChannel(): Promise<boolean> {
		...
		try {
			await this.waitingRoomChannel.guild.channels.fetch(
				this.waitingRoomChannel.id,
			);
			return true;
		} catch {
			logger.info(
	...
	async checkWaitingRoomMessage(): Promise<Message<true> | undefined> {
		...
		try {
			return await this.waitingRoomChannel.messages.fetch(
	...
	async checkIfWaitingRoomChannelExists(
	...
	async sendWaitingRoomEmbed(
		...
		const channelExists =
			await this.checkIfWaitingRoomChannelExists(deleteRoom);
	...
	async execWin(): Promise<void> {
		if (
			!this.waitingRoomChannel ||
			!(this.waitingRoomChannel instanceof TextChannel)
		) {
			logger.warn(
				`Invalid waiting room channel: ${
					this.waitingRoomChannel ?? 'undefined'
				}`,
			);
			return;
		}
		...
		const winningEmbeds = await Promise.all(
			this.players.map(async (player) => {
				...
				if (player.isWinner) {
					const isWinnerEmbed = await doEmbed<Player>(
						GameStatus.win,
						this,
						player,
					);
