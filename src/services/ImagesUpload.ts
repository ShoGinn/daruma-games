...
export class ImagesUpload {
  ...
  async syncWithDatabase() {
    ...
    for (const image of imagesInDb) {
      const imagePath = `${image.basePath !== '' ? `${image.basePath}/` : ''}${
        image.fileName
      }`
    ...
    for (const imagePath of images) {
      ...
      if (!imageInDb) await this.addNewImageToImgur(imagePath, imageHash)
        ...
        console.warn(
          `Image ${chalk.bold.green(
            imagePath
          )} has the same hash as ${chalk.bold.green(
            `${imageInDb.basePath}${imageInDb.basePath?.length ? '/' : ''}${
              imageInDb.fileName
            }`
          )} so it will skip`
        )
  ...
  async addNewImageToImgur(
    ...
    try {
      ...
      await this.logger.log(error?.toString(), 'error', true)
    }
  }
