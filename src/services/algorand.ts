...
export class Algorand extends AlgoClientEngine {
	...
	getAccountFromMnemonic(mnemonic: string): Account | undefined {
		...
		try {
			...
			logger.error(
				`Failed to get account from mnemonic: ${JSON.stringify(error)}`,
			);
			return;
		}
		return secretKey;
	...
	getMnemonicAccounts(
		...
		claimTokenMnemonic: string | undefined = getConfig().get(
			'claimTokenMnemonic',
		),
	...
	public async getAccountAssets(
		...
		const assets = accountAssets[assetType] as
			| AssetHolding[]
			| MainAssetResult[];
		...
		if (assetType === 'created-assets') {
			logger.info(
				`Found ${assets.length} ${assetType} for account ${walletAddress}`,
			);
		}
	 ...
	 * @memberof Algorand
	 */
	public async lookupAssetsOwnedByAccount(
		walletAddress: string,
	): Promise<AssetHolding[]> {
		return (await this.getAccountAssets(
			walletAddress,
			'assets',
		)) as AssetHolding[];
	}
	 ...
	 * @memberof Algorand
	 */
	public async getCreatedAssets(
		walletAddress: string,
	): Promise<MainAssetResult[]> {
		return (await this.getAccountAssets(
			walletAddress,
			'created-assets',
		)) as MainAssetResult[];
	...
	async getAssetArc69Metadata(
		assetIndex: number,
	): Promise<Arc69Payload | undefined> {
		try {
			...
			logger.error(
				`Error fetching metadata for assetIndex ${assetIndex}:`,
				error,
			);
			return undefined;
		}
			...
			if (claimStatus.txId) {
				logger.info(
					`Auto Claimed ${
						...
						claimStatus?.status?.['confirmed-round'] ?? 'unk'
	 ...
	 * @memberof Algorand
	 */
	private logFailedChunkedWallets(
		chunk: Array<[AlgoWallet, number, string]>,
	): void {
		for (const wallet of chunk) {
	...
	async assetTransfer(
		...
		try {
			const suggestedParameters = await this.algodClient
			...
			if (senderAddress.length > 0) {
				...
				const { tokens: senderBalance } = await this.rateLimitedRequest(
					async () => {
			...
			if (!groupTransfer || groupTransfer?.length === 1) {
				...
				rawTxn = await this.algodClient
					.sendRawTransaction(rawSingleSignedTxn)
					.do();
			} else {
				...
				const rawMultiSignedTxn = rawMultiTxn.map((txn) =>
					txn.signTxn(signTxnAccount),
				);
				rawTxn = await this.algodClient
					.sendRawTransaction(rawMultiSignedTxn)
					.do();
			}
