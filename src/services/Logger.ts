...
export class Logger {
  ...
  async logInteraction(interaction: AllInteractions) {
    ...
    const chalkedMessage = oneLine`
            ...
            ${
              channel instanceof TextChannel || channel instanceof ThreadChannel
                ? `${chalk.dim.italic.gray('in channel')} ${chalk.bold.blue(
                    `#${channel.name}`
                  )}`
                : ''
            }
            ${
              guild
                ? `${chalk.dim.italic.gray('in guild')} ${chalk.bold.blue(
                    `${guild.name}`
                  )}`
                : ''
            }
            ${
              user
                ? `${chalk.dim.italic.gray('by')} ${chalk.bold.blue(
  ...
  async logNewUser(user: User) {
    ...
    const chalkedMessage = `(${chalk.bold.white(
      'NEW_USER'
    )}) ${chalk.bold.green(user.tag)} (${chalk.bold.blue(
      user.id
    )}) ${chalk.dim.italic.gray('has been added to the db')}`
  ...
  async logGuild(
    ...
    await resolveDependency(Client).then(async client => {
      ...
      const message = `(${type}) Guild ${
        guild ? `${guild.name} (${guildId})` : guildId
      } ${additionalMessage}`
      const chalkedMessage = oneLine`
                (${chalk.bold.white(type)})
                ...
                ${
                  guild
                    ? `${chalk.bold.green(guild.name)} (${chalk.bold.blue(
                        guildId
                      )})`
                    : guildId
                }
  ...
  async logError(
    ...
    if (trace && trace[0]) {
      message += ` ${
        type === 'Exception' ? 'Exception' : 'Unhandled rejection'
      } : ${error.message}\n${trace
        .map((frame: StackFrame) => `\t> ${frame.file}:${frame.lineNumber}`)
        .join('\n')}`
      embedMessage += `\`\`\`\n${trace
        .map((frame: StackFrame) => `> ${frame.file}:${frame.lineNumber}`)
        .join('\n')}\n\`\`\``
      ...
      chalkedMessage += ` ${chalk.dim.italic.gray(
        type === 'Exception' ? 'Exception' : 'Unhandled rejection'
      )} : ${error.message}\n${chalk.dim.italic(
        trace
          .map((frame: StackFrame) => `\t> ${frame.file}:${frame.lineNumber}`)
  ...
  logStartingConsole() {
    ...
    const pluginsEntitiesCount = this.pluginsManager.plugins.reduce(
    ...
    this.console(
      chalk.red(
        `${symbol} ${numberAlign(
          entities.length + pluginsEntitiesCount
        )} ${chalk.bold('entities')} loaded`
      ),
      'info',
      true
    )
    ...
    // services
    const services = fs
      .readdirSync(`${__dirname}/../services`)
    ...
    this.console(
      chalk.yellow(
        `${symbol} ${numberAlign(
          services.length + pluginsServicesCount
        )} ${chalk.bold('services')} loaded`
      ),
      'info',
      true
    )
    ...
    if (apiConfig.enabled) {
      ...
      this.console(
        chalk.cyan(
          `${symbol} ${numberAlign(endpointsCount)} ${chalk.bold(
            'api endpoints'
          )} loaded`
        ),
        'info',
    ...
    this.console(
      chalk.green(
        `${symbol} ${numberAlign(scheduledJobs)} ${chalk.bold(
          'scheduled jobs'
        )} loaded`
      ),
      'info',
    ...
    this.console(
      chalk.hex('#47d188')(
        `${symbol} ${numberAlign(pluginsCount)} ${chalk.bold(
          'plugin' + (pluginsCount > 1 ? 's' : '')
        )} loaded`
      ),
      'info',
      true
    ...
    this.console(
      chalk.hex('7289DA')(
        boxen(
          ` ${
            this.client.user ? `${chalk.bold(this.client.user.tag)}` : 'Bot'
          } is ${chalk.green('connected')}! `,
          {
            padding: 0,
            margin: {
              top: 1,
