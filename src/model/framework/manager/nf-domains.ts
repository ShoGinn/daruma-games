...
export class NFDomainsManager extends AbstractRequestEngine {
  ...
  public async getNFDRecordsOwnedByWallet(algorandWalletAddr: string): Promise<NFDRecordsByWallet> {
  ...
  public isNFDWalletVerified(algorandWalletAddr: string, NFDRecords?: NFDRecordsByWallet): boolean {
    if (!NFDRecords) {
      return false;
    }
