...
export class AlgoNFTAssetRepository extends EntityRepository<AlgoNFTAsset> {
	...
	async persistBulkArc69(
		...
		for (const assetEntity of assetEntities) {
			...
			const updatedAsset = assetsWithUpdates.find(
				(asset) => asset.id === assetEntity.id,
			);
			if (updatedAsset) {
	...
	async creatorAssetSync(): Promise<void> {
		...
		const creatorAddressArray = await em
	...
	async addAssetsLookup(
		...
		const filteredAssets = creatorAssets.filter(
			(asset) =>
				!existingAssets.some(
					(existingAsset) => existingAsset.id === asset.index,
				),
		);
	...
	async assetRankingByWinsTotalGames(): Promise<Array<AlgoNFTAsset>> {
		...
		const sortedAssets: Array<AlgoNFTAsset> | undefined =
			customCache.get('rankedAssets');
		if (sortedAssets) {
			return sortedAssets;
		}
	...
	async getBonusData(
		...
		if (!gameBonusData) {
			...
			const averageRank =
				Math.round(sumOfRanks / rankedAssetsSorted.length) || 1;
