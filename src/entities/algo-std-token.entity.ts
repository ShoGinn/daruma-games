...
export class AlgoStdTokenRepository extends EntityRepository<AlgoStdToken> {
	...
	async addAlgoStdToken(
		...
		const tokens = ObjectUtil.convertBigIntToNumber(
			liveToken.tokens,
			asset.decimals,
		);
	...
	async doesWalletHaveAsset(
		...
		return !!wallet.asa
			.getItems()
			.some((walletAsset) => walletAsset.id === assetIndex);
	}
	...
	async isWalletWithAssetOptedIn(
		wallet: AlgoWallet,
		assetIndex: number,
	): Promise<boolean> {
		await wallet.asa.init();
	...
	async getAllAssetsByWalletWithUnclaimedTokens(
		wallet: AlgoWallet,
	): Promise<AlgoStdToken[]> {
	...
	async removeUnclaimedTokens(
		...
		const em = this.getEntityManager();
		const walletHasAsset = await this.getWalletWithUnclaimedTokens(
			wallet,
			assetIndex,
		);
		if (walletHasAsset) {
