...
import {
	gameStatusHostedUrl,
	getAssetUrl,
} from '../utils/functions/dt-images.js';
...
export interface AllWalletAssetsAdded {
	assetsUpdated:
		| { assetsAdded: number; assetsRemoved: number; walletAssets: number }
		| undefined;
...
export class AlgoWalletRepository extends EntityRepository<AlgoWallet> {
	...
	async anyWalletsUpdatedMoreThan24HoursAgo(): Promise<boolean> {
		...
		const filteredWallets = wallets.filter(
			(wallet) => !internalUserIds.has(wallet.owner.id),
		);
	 ...
	 * @memberof AlgoWalletRepository
	 */
	async getWalletsWithNFTsLoaded(
		walletAddress: string,
	): Promise<Loaded<AlgoWallet, 'nft'>> {
	 ...
	 * @memberof AlgoWalletRepository
	 */
	async getAllWalletsByDiscordId(
		discordId: string,
	...
	async addCreatorWallet(walletAddress: string): Promise<AlgoWallet | null> {
		return await this.addInternalUserWallet(
			walletAddress,
	...
	async addInternalUserWallet(
		...
		let user = await em
			.getRepository(User)
			.findOne({ id: internalUser.toString() });
		if (!user) {
	...
	async removeCreatorWallet(walletAddress: string): Promise<void> {
		return await this.removeInternalUserWallet(
			walletAddress,
			InternalUserIDs.creator,
		);
	}
	...
	async getWalletStdAsset(
		...
		return (
			walletEntity.asa.getItems().find((asset) => asset.id === stdAssetId) ??
			null
		);
	}
	/**
	 ...
	 * @memberof AlgoWalletRepository
	 */
	async addAllAlgoStdAssetFromDB(
		walletAddress: string,
	): Promise<Array<AlgoStdAssetAdded>> {
		const em = container.resolve(MikroORM).em.fork();

		// Get all the ASA's registered to the bot
		const algoStdAssets = await em
	 ...
	 * @memberof AlgoWalletRepository
	 */
	generateStringFromAlgoStdAssetAddedArray(
		array: Array<AlgoStdAssetAdded>,
	): string {
		return array
			.map((asset) =>
				inlineCode(
	 ...
	 * @memberof AlgoWalletRepository
	 */
	async addAllAssetsToWallet(
		walletAddress: string,
	): Promise<AllWalletAssetsAdded> {
		const algorand = container.resolve(Algorand);
		const algorandAssets =
			await algorand.lookupAssetsOwnedByAccount(walletAddress);

		const assetsUpdated = await this.addWalletAssets(
			walletAddress,
			algorandAssets,
		);
		const asaAssetsAdded = await this.addAllAlgoStdAssetFromDB(walletAddress);
		const asaAssetsString =
			this.generateStringFromAlgoStdAssetAddedArray(asaAssetsAdded);
	...
	> {
		...
		try {
			...
			for (const assetId of allAssetIds) {
				...
				const holderAsset = holderAssets.find(
					(asset) => asset['asset-id'] === assetId,
				);

				if (
					holderAsset &&
					holderAsset.amount > 0 &&
					!walletAsset &&
					creatorAsset
				) {
	 ...
	 * @memberof AlgoWalletRepository
	 */
	async getTokensAddedToWallet(
		walletAddress: string,
	): Promise<Array<AlgoStdToken>> {
		const wallet = await this.findOneOrFail(
			{ address: walletAddress },
			{ populate: ['tokens'] },
		);
	...
	async allWalletsOptedIn(
		...
		for (const currentWallet of wallets) {
			const walletTokens = await this.getTokensAddedToWallet(
	...
	async createNPCsIfNotExists(): Promise<boolean> {
		...
		for (const bot of gameNPCs) {
			...
			await em
				.getRepository(AlgoNFTAsset)
				.createNPCAsset(botCreatorWallet, newAsset);
		}
	...
	async topNFTHolders(): Promise<Map<string, number>> {
		...
		const em = container.resolve(MikroORM).em.fork();
		let topNFTHolders = (await cache.get(
		...
		if (!topNFTHolders) {
			...
			for (const user of allUsers) {
				const allWallets = await this.getAllWalletsAndAssetsByDiscordId(
					user.id,
				);
				// Count total NFT in wallet
				const totalNFT = allWallets.reduce(
					(total, wallet) => total + wallet.nft.length,
					0,
				);
				if (totalNFT > 0) {
