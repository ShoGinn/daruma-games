...
export class AlgoWalletRepository extends EntityRepository<AlgoWallet> {
  ...
  async randomAssetCoolDownReset(
    ...
    allAssets = RandomUtils.random.shuffle(allAssets);
   ...
   * @memberof AlgoWalletRepository
   */
  async getWalletStdAsset(walletAddress: string, stdAssetId: number): Promise<AlgoStdAsset | null> {
  ...
  async allWalletsOptedIn(discordUser: string, stdAsset: AlgoStdAsset | string): WalletTokens {
    ...
    for (const currentWallet of wallets) {
      ...
      for (const walletToken of walletTokens) {
        ...
        if (stdAsset && walletToken.asa[0]?.unitName == stdAsset.unitName && walletToken.optedIn) {
  ...
  async topNFTHolders(): Promise<Map<string, number>> {
    ...
    const em = container.resolve(MikroORM).em.fork();
    let topNFTHolders = (await cache.get(DarumaTrainingCacheKeys.TOP_NFT_HOLDERS)) as Map<
      string,
      number
    >;
    if (!topNFTHolders) {
