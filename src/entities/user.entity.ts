...
export class UserRepository extends EntityRepository<User> {
	...
	async isWalletOwnedByOtherDiscordID(
		...
		return await nfDomainsMgr.isWalletOwnedByOtherDiscordID(
			discordUser,
			wallet,
		);
	}
	...
	private __processWalletOwnerMsg(
		walletAddress: string,
	...
	async addWalletAndSyncAssets<T extends string | User>(
		...
		const walletOwners = await this.addNewWalletToUser(
			discordUser,
			walletAddress,
		);
		...
		const { assetsUpdated, asaAssetsString } =
			await this.addAllAssetsToWallet(walletAddress);
